...
export class GameScene extends Phaser.Scene {
  ...
  private showResultPanel(cmp: GhostComparison, myDist: number) {
    // 수정: 최종 등수 등급 칭호 — finalRank가 RANK_TITLES 길이를 넘어가더라도
    // 배열 인덱스 초과를 방지하고 가장 하위 칭호로 폴백하도록 Math.min 적용
    const finalRank = cmp.total - cmp.overtaken + 1;
    const rankIndex = Math.min(finalRank - 1, this.RANK_TITLES.length - 1);
    const rankTitle = this.RANK_TITLES[rankIndex];
  ...
  private syncVisuals() {
    ...
    // 수정: 5콤보마다 마일스톤 팝업 — combo가 COMBO_MILESTONES 길이를 넘어가더라도
    // 인덱스 초과가 발생하지 않도록 Math.min으로 가장 높은 칭호를 계속 유지합니다.
    if (s.combo > this.prevCombo && s.combo % 5 === 0) {
      const tier = Math.floor(s.combo / 5);
      const milestoneIndex = Math.min(tier - 1, this.COMBO_MILESTONES.length - 1);
      const milestone = this.COMBO_MILESTONES[milestoneIndex];
